...
class scenario:
    ...
    def battle1(self, pzd):
        pzd.useSkill(0, msg="シヴィニア1変身", wait=3)
        pzd.useSkill(4, msg="シヴィニア2変身", wait=3)
        pzd.useSkill(1, msg="ポロネスキル使用", wait=3)
        self.useSkillAndMove(0,msg="シヴィニアスキル使用")
    
    # 2Fの処理
    def battle2(self, pzd):
        self.useSkillAndMove(4,msg="シヴィニアスキル使用")
    ...
    def battle4(self, pzd):
        pzd.wait(2, msg="落ちコン回避の待ち(毒生成によりコンボしやすいのかも)")
        self.useSkillAndMove(4,msg="シヴィニアスキル使用")

    # 5Fの処理
    def battle5(self, pzd):
        pzd.wait(10, msg="敵のスキル使用を待つ")
        pzd.useSkill(2, msg="ラファエルスキル使用", wait=3)
        self.useSkillAndMove(0,msg="シヴィニアスキル使用")

    # 5F(ボス)の処理
    def battleBoss(self, pzd):
        pzd.wait(7, msg="敵のスキル使用を待つ")
        pzd.useSkill(4, msg="シヴィニア1スキル使用", wait=2.5)
